...
import { fetchBlogBySlug, fetchBlogs, submitLead, BlogItem } from '@/data/faisalHillsData';
...
export default function BlogDetailPage() {
  ...
  // FAQ state
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(0); // default open first FAQ
  ...
  useEffect(() => {
    ...
    fetchBlogBySlug(slug)
      .then((data) => {
        ...
        setLoading(false);
        if (data) {
          document.title = `${data.metaTitle || data.title} | Faisal Hills Real Estate`;
          const metaDesc = document.querySelector('meta[name="description"]');
          if (metaDesc) {
            metaDesc.setAttribute('content', data.metaDescription || data.summary);
          }
        }
      })
      .catch((err) => {
  ...
  const handleConsultationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Save locally in local storage for admin logs
  ...
  if (loading) {
    return (
      <div className="bg-[#fff8f6] min-h-screen py-24 flex items-center justify-center flex-col space-y-3 font-sans">
        <div className="w-12 h-12 border-4 border-[#7b002c] border-t-transparent rounded-full animate-spin" />
        <p className="text-slate-500 font-semibold text-xs">Loading article details...</p>
      </div>
    );
  }
  ...
  if (!blog) {
    return (
      <div className="bg-[#fff8f6] min-h-screen py-20 font-sans flex items-center justify-center">
        <div className="max-w-md w-full bg-white p-8 rounded-3xl border border-slate-200 shadow-xl text-center space-y-5">
          ...
          <h2 className="font-serif font-black text-xl text-slate-900">Article Not Found</h2>
          <p className="text-slate-500 text-xs leading-relaxed">
            The blog article you are looking for does not exist, has been unpublished, or was moved.
          </p>
          <button
            onClick={() => router.push('/blogs')}
            className="w-full py-3 bg-[#7b002c] hover:bg-[#9e1245] text-white font-bold text-xs rounded-xl shadow-md transition cursor-pointer"
          >
  ...
  // Get other recent blogs (excluding current one)
