...
export default function PlotDetailClient({ plotId: initialPlotId }: PlotDetailClientProps) {
  ...
  if (!currentPlot) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8 pt-32">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center max-w-md shadow-sm space-y-4">
          <div className="w-12 h-12 rounded-full bg-rose-50 text-[#7b002c] flex items-center justify-center mx-auto">
            <Home className="w-6 h-6" />
          </div>
          <h2 className="text-xl font-bold font-serif text-slate-900">Plot Listing Details</h2>
          <p className="text-xs text-slate-500 leading-relaxed">
            We are fetching the latest information for this plot from our live inventory. If the plot was recently added, please explore the complete directory below.
          </p>
          <div className="pt-2 flex justify-center gap-3">
            <Link
              href="/plots"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#7b002c] text-white text-xs font-bold hover:bg-[#9e1245] transition"
            >
              <ArrowLeft className="w-4 h-4" />
              <span>Browse All Plots</span>
            </Link>
          </div>
        </div>
      </div>
