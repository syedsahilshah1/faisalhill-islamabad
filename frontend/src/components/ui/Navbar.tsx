...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        {/* Inner Floating Navbar Wrapper - Expands to edge-to-edge transparent at top, becomes rounded white pill when scrolled */}
        <div
        ...
        >
            ...
            <Link
              ...
              className={`text-xs xl:text-[13px] 2xl:text-sm font-bold transition-colors py-1 relative whitespace-nowrap ${
                pathname === '/about-us'
                  ...
                  : isScrolled ? 'text-slate-800 hover:text-[#7b002c]' : 'text-white hover:text-white/80'
              }`}
            >
              <span>About Us</span>
            </Link>
          ...
          >
            {mobileMenuOpen ? (
              <X className={`w-5 h-5 sm:w-6 sm:h-6 ${isScrolled ? 'text-[#7b002c]' : 'text-white'}`} />
            ) : (
              <Menu className={`w-5 h-5 sm:w-6 sm:h-6 ${isScrolled ? 'text-[#7b002c]' : 'text-white'}`} />
            )}
          </button>
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden fixed inset-0 top-16 sm:top-20 bg-slate-900/80 backdrop-blur-md z-[999] flex flex-col justify-start">
            <div className="bg-white text-slate-900 border-b-4 border-[#7b002c] p-6 space-y-5 shadow-2xl max-h-[80vh] overflow-y-auto">

              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <span className="text-xs font-bold text-[#7b002c] uppercase tracking-wider">Navigation Menu</span>
                <button
                  type="button"
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-slate-500 hover:text-slate-800 p-1 font-bold text-xs bg-slate-100 rounded-lg px-2"
                >
                  ✕ Close
                </button>
              </div>
