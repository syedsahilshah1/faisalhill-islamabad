...
export default function WhatsAppButton() {
  ...
  // Show assist message 2s after button appears, auto-hide after 8s
  useEffect(() => {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-[9999] flex items-end gap-3 transition-all duration-500 ${
        visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'
      }`}
    >
      {/* Assist Message Bubble */}
      <div
        className={`relative bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 max-w-[240px] transition-all duration-400 origin-bottom-right ${
      ...
      >
        ...
        <button
          ...
          className="absolute -top-2 -right-2 w-5 h-5 bg-slate-800 text-white rounded-full flex items-center justify-center hover:bg-slate-600 transition-colors cursor-pointer"
        >
          <X className="w-3 h-3" />
        </button>

        {/* Bubble tail */}
        <div className="absolute bottom-3 -right-2 w-4 h-4 bg-white border-r border-b border-slate-200 rotate-[-45deg]" />
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5 leading-snug">
              Hi! 👋 Need help with plots, prices, or payment plans? Chat with us now!
            </p>
          </div>
        </div>
      </div>
      ...
      <a
        ...
        className="group relative w-14 h-14 bg-[#25D366] hover:bg-[#20bd5a] rounded-full flex items-center justify-center shadow-[0_4px_20px_rgba(37,211,102,0.4)] hover:shadow-[0_6px_28px_rgba(37,211,102,0.55)] transition-all duration-300 hover:scale-110 active:scale-95"
